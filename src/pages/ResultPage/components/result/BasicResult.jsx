...
export const BasicResult = ({basic}) => {
    return (
        <BasicResultContainer>
            ...
            <Box>
                <BoxTitle>경험치</BoxTitle><BoxValue>{basic.characterExp.replace(/\B(?=(\d{3})+(?!\d))/g,',')} EXP</BoxValue>
            </Box>
            <Box>
                <BoxTitle>생성 시각</BoxTitle><BoxValue>{formatDate(new Date(basic.characterDateCreate))}</BoxValue>
            </Box>
        </BasicResultContainer>
    )
}